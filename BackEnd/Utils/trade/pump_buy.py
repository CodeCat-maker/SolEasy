    ...
    async with AsyncClient(RPC_ENDPOINT) as client:
        ...
        for ata_attempt in range(max_retries):
            try:
                ...
                if account_info.value is None:
                    ...
                    create_ata_tx = Transaction([payer], msg, recent_blockhash.value.blockhash)

                    await client.send_transaction(create_ata_tx)
        ...
        for attempt in range(max_retries):
            try:
                ...
                msg = Message([buy_ix], payer.pubkey())
                recent_blockhash = await client.get_latest_blockhash()
                transaction = Transaction([payer], msg, recent_blockhash.value.blockhash)
